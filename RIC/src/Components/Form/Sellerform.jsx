...
const Sellerform = () => {
  ...
  const handleAmenityChange = (amenity) => {
    setSelectedAmenities((prev) => {
      const updated = { ...prev };
      if (updated[amenity]) {
        delete updated[amenity]; // Uncheck main amenity
      } else {
        updated[amenity] = []; // Initialize with an empty array
      }
      return updated;
    });
  };
  const handleSubcategoryChange = (amenity, subcategory) => {
    setSelectedAmenities((prev) => {
      const updated = { ...prev };

      // Ensure the main category exists
      if (!updated[amenity]) {
        updated[amenity] = [];
      }

      // Add or remove subcategory
      if (updated[amenity].includes(subcategory)) {
        updated[amenity] = updated[amenity].filter((item) => item !== subcategory);
      } else {
        updated[amenity] = [...updated[amenity], subcategory];
      }
...
<div className="amenity-container">
  {Object.keys(amenitiesData).map((amenity) => (
    <div key={amenity}>
      {/* Main Amenity Checkbox */}
      ...
      {/* Subcategories (Appearing below main heading) */}
      {selectedAmenities[amenity] && (
        <div className="subcategory-container">
          {amenitiesData[amenity].map((sub, idx) => (
            <div key={idx} className="subcategory-item">
              <input
                type="checkbox"
                id={sub}
                checked={selectedAmenities[amenity]?.includes(sub)}
                onChange={() => handleSubcategoryChange(amenity, sub)}
              />
